...
La classe AfficherProduitsSwing représente une interface graphique Swing permettant d’afficher une liste de produits depuis une base de données et de filtrer cette liste à l’aide d’un champ de recherche. Elle hérite de JFrame et utilise un DefaultTableModel pour gérer les données d’un tableau JTable. Dès son initialisation, la fenêtre configure son titre, sa taille, et place manuellement ses composants à l’aide de setBounds. Elle affiche une étiquette et un champ texte pour permettre la recherche en temps réel, ainsi qu’un tableau affichant les produits (ID, Nom, Quantité, Prix). Les données sont récupérées via ProduitDAO et stockées dans la variable tousLesProduits. À chaque modification du champ de recherche, un filtre s’applique automatiquement grâce à un DocumentListener, qui recharge dans le tableau uniquement les lignes correspondant à l’ID ou au nom recherché. De plus, lorsque l’utilisateur clique sur une ligne du tableau, la classe récupère les données de cette ligne et ouvre une fenêtre ModifierOuSupprimerProduit avec les détails du produit sélectionné. Deux méthodes privées, afficherProduits(...) et filtrer(), gèrent respectivement l’affichage brut des données et leur filtrage dynamique.
...
```
# Explication de la classe
La classe MenuPrincipal constitue la fenêtre principale de l'application de gestion de stock. Elle hérite de JFrame et utilise les composants Swing pour construire l'interface graphique. Lors de l'initialisation, elle configure la fenêtre : titre, taille, centrage, et couleur de fond pastel. Un logo est chargé et affiché en haut, suivi d’un titre centré. Ensuite, un tableau de noms est utilisé pour créer dynamiquement une série de boutons alignés verticalement, chacun représentant une action spécifique comme : ajouter, afficher, modifier ou supprimer un produit, gérer les mouvements de stock, voir l'historique, générer un rapport PDF ou afficher des statistiques. Chaque bouton est lié à une action précise par un ActionListener, ouvrant une fenêtre ou exécutant une fonction. Notamment, le bouton "Exporter PDF" appelle une méthode privée exporterPDF(), qui utilise JasperReports pour compiler un rapport .jrxml, le remplir avec les données de la base, et l’enregistrer en PDF sur le disque. Enfin, la méthode main() lance l'interface en instanciant MenuPrincipal.
